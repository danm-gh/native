...
/**
 * Keep the model-readable docs index on the same inventory as the sidebar.
 * Each target is the clean Markdown representation generated from its page.mdx
 * source at /<page path>.md, so neither this index nor its linked content needs
 * a second hand-maintained copy.
 */
export const dynamic = "force-static";

export function GET() {
  const sections = navSections.map(({ title, items }) => {
