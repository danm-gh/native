...
pub const on_drag_payload_message = "on-drag takes a Msg tag plus one integer source binding (for example card_dragged:{card.id}); that Msg arm must be exactly { sourceId, phase, x, y, viewWidth, viewHeight } with numeric fields - markup fills sourceId and the runtime fills phase (0 change, 1 end, 2 cancel) plus live geometry";
