...
Window creation accepts four orthogonal overlay options:

- `transparent` makes the top-level window and its render surface alpha-capable. A canvas view must also use a non-opaque `gpu_alpha_mode`, and its clear/content alpha determines what reaches the desktop.
...
On macOS these map to an alpha-capable floating `NSWindow`, ignored mouse events, and passive `orderFront`. Windows uses a layered topmost window with per-pixel alpha, transparent hit testing, `WS_EX_NOACTIVATE`, and an atomic first-present reveal. Its layered presenter composites multiple canvas surfaces in layer order; WebViews and other native views are rejected inside a transparent Windows window because Win32 cannot redirect those child surfaces into the top-level alpha bitmap. Linux provides transparency, input regions, and passive show on GTK; topmost is honored on X11 through `_NET_WM_STATE_ABOVE`. Wayland intentionally gives the compositor—not clients—control of topmost placement, so the GTK host reports that limitation and cannot guarantee `always_on_top` there.
