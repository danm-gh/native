...
test "native file drops add cards and card drags preview and commit arbitrary insertion" {
    ...
    try std.testing.expect(h.hasText("2 todo · 1 doing · 2 done · 0 added from files"));
    ...
    try std.testing.expect(h.hasText("4 todo · 1 doing · 2 done · 2 added from files"));
    ...
    const sketch_before = findCard(h.app_state.tree.?.root, "Sketch the board layout").?;
    ...
    const sketch_pushed = findCard(h.app_state.tree.?.root, "Sketch the board layout").?;
    try std.testing.expect(destination_frame.y < (try h.frameFor(sketch_pushed.id)).y);
    try std.testing.expectApproxEqAbs(@as(f32, 33.5), destination_frame.height, 0.01);
    ...
    const first_done = findCard(h.app_state.tree.?.root, "Copy inbox scaffolding").?;
    const second_done = findCard(h.app_state.tree.?.root, "Read the builder source").?;
    ...
    const first_done_preview = findCard(h.app_state.tree.?.root, "Copy inbox scaffolding").?;
    const second_done_pushed = findCard(h.app_state.tree.?.root, "Read the builder source").?;
    ...
    try std.testing.expect(h.hasText("3 todo · 1 doing · 3 done · 2 added from files"));
