...
The flagship [`examples/ai-chat-ts`](https://github.com/vercel-labs/native/tree/main/examples/ai-chat-ts) keeps the buffered overload to demonstrate whole-response JSON parsing and single-result replay: conversation history in the Model, endpoint and credentials through the env channel, and a runtime-built `Authorization: Bearer` header, with an end-to-end suite that pins the request and replays it without network or launch variables. Streaming is an app-level choice, not a different core language or runtime.
