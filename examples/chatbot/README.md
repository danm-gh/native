...
The example defaults to `openai/gpt-5.6-luna`. To use another Gateway model, add `NATIVE_SDK_CHAT_MODEL="<creator/model>"` to the `native dev` command.
...
- **Long conversations eventually hit the request bound.** The engine's fetch body bound is 64 KiB; a conversation that outgrows it is rejected by the engine at runtime and lands in the failed state with a reason. New chat starts fresh. (History trimming/summarizing is app policy, deliberately not built in here.)
