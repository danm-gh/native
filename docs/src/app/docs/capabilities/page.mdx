...
Native SDK capabilities are native OS services and app events exposed through `PlatformServices`, runtime methods, lifecycle events, and — for apps that [embed web content](/docs/frontend) — guarded bridge commands. Native code reaches them directly; web content does not receive capability access by default. In a [`UiApp`](/docs/app-model), notifications and clipboard access ride the effects channel (`fx.showNotification`, `fx.writeClipboard`, and `fx.readClipboard`) so `update` never needs a runtime handle.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td><code>fx.showNotification(options)</code> / <code>runtime.showNotification(options)</code></td>
...
Model-driven apps request a notification from an `update_fx` arm. Delivery is fire-and-forget because OS focus modes and user settings remain authoritative after the host accepts the request; fake-executor tests and session replay do not display notifications.
